...
fn format_code(value: &str) -> String {
    let syntax_tree = syn::parse_file(value).unwrap();
...
fn build_byte1_match(x: Byte1Map, target: &mut Block) {
    for (first_byte, end_byte_to_codepoint) in x {
        let mut body = Block::new(&format!("({:#2X}, _) =>", first_byte));
        body.line("*bytes = rest;");
        body.line(&format!("{:#2x}", end_byte_to_codepoint));
...
type Byte1Map = BTreeMap<u8, u8>;
type Byte2Map = BTreeMap<u8, Vec<(u8, u8)>>;
type Byte3Map = BTreeMap<u8, BTreeMap<u8, Vec<(u8, u8)>>>;

fn build_encoder_internal(name: &str, definition: &UnicodeMapping, ebcdic: bool) -> Impl {
    let mut buffer = [0u8; 4];
    let mut bytes1: Byte1Map = BTreeMap::new();
    ...
    if ebcdic {
        build_byte1_match(bytes1, &mut match_body);
    } else {
    ...
    let ebcdic_fn = res.new_fn("is_ebcdic");
    ebcdic_fn
        .arg_ref_self()
        .ret("bool")
        .attr("inline")
        .line(ebcdic.to_string());
    res
}

fn generate_coder(name: &str, definition: UnicodeMapping, ebcdic: bool) -> Result<()> {
    let mut file = BufWriter::new(
        File::create(format!("../src/code_pages/{}.rs", name.to_lowercase())).unwrap(),
    );
    let mut coder = Scope::new();
    coder.push_impl(build_encoder_internal(name, &definition, ebcdic));
    ...
    if definition
        ...
        .take(128)
        .enumerate()
        .any(|(i, c)| i < 128 && c.map(|c| !c.is_ascii() || ebcdic).unwrap_or(true))
    {
        //non-ascii mapping
...
fn main() -> Result<()> {
    ...
    for cp in unicode_dot_org_encodings {
        ...
        generate_coder(&name, definition, false)?;
    }
    ...
    generate_coder("CP037", rfc1345::parse_ibm037(), true)?;
    generate_coder("CP1140", rfc1345::parse_ibm1140(), true)?;
